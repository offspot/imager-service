...
from manager.pibox.data import ideascube_languages, YAML_CATALOGS
from manager.pibox.util import b64decode, human_readable_size
...
def get_packages_id():
    return [package.get('langid') for package in ALL_PACKAGES
            if package.get('langid')]


def get_packages_langs():
    _name = lambda lk, ln: ln if ln else lk.upper()
