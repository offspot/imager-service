...
from manager.pibox import PACKAGES_BY_LANG
...
def packages_for_language(request, lang_code):
    def _filter(package):
        return {k: v for k, v in package.items()
                if k in ('sname', 'hsize', 'tags', 'version',
                         'type', 'description', 'size', 'skey')}
    ordered = collections.OrderedDict(sorted([
        (k, _filter(v))
        for k, v in PACKAGES_BY_LANG.get(lang_code, []).items()
...
def required_size_for_config(request):
    ...
    # check disk space
    collection = get_collection(edupi=data.get('edupi', False),
                                edupi_resources=data.get('edupi_resources',
                                                         None),
                                packages=data.get('packages', []),
                                kalite_languages=data.get('kalite', []),
                                wikifundi_languages=data.get('wikifundi', []),
                                aflatoun_languages=['fr', 'en']
                                if data.get('aflatoun', False) else [])
